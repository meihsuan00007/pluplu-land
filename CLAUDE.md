...
- **首頁區塊順序**（2026-08-31 主理人指定，由上而下 10 個區塊，改版時勿自行調換）：
  頂部輪播 → 創辦人引言 → 三大分類入口方塊 → 分享優惠橫幅 → Lookbook 小基地 → 品牌情境標語（含 01/02/03 理念三欄）
  → 大家的心頭好 → 野餐季企劃（`#picnic-plan`）→ 秋日針織企劃（`#knit-collection`）→ 品牌故事引導（深色）。
  間距規格：輪播後第一個區塊 60px（手機 30px）；「三大方塊→優惠橫幅→Lookbook」三格緊貼（各間隔 100px，後兩者掛 `padding-top:0`）；
  其餘區塊維持全站標準 200px＋stitch 分隔線。
- **首頁三大分類入口方塊的封面每次重新整理隨機換一張**（2026-08-31 主理人指定，邏輯在 `core/covers.ts`）：
  新品上市＝目錄最新 5 件商品、織女手作系列＝tags 含「織女手工系列」、配件專區＝`accessory` 分類；
  抽法是**先平均抽一件商品、再抽那件商品的封面照片**，照片多的商品不會比較容易被抽中（每件曝光機會均等）；
  **每件商品只認主圖**（相簿第一張），不從整本相簿抽，才不會抽到白底單品照或局部細節特寫這種不適合當封面的照片
  （2026-08-31 主理人指定）。所以換某件商品的主圖，就等於換掉它在首頁方塊可能出現的封面。
  後台 `site.json` 的 banner 圖片只在「商品資料還沒載入完」時顯示，改那張圖不會改變正式封面。
  取樣分類是看方塊連結的 `?cat=` 決定的，後台改連結時封面來源會跟著換。
  「新品上市」方塊連 `/shop?sort=new`：進選品頁會捲到陳列架並自動切成「上架順序：由新到舊」。
