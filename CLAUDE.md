...
- **首頁創辦人金句在手機上必須「一句一行」**（2026-09-18 主理人指定）：不可讓「生命。」「點。」這種兩三個字掉成孤行。
  做法是 560px 以下把金句區左右留白收成 12px，字級改成跟著螢幕寬度縮放（`min(20px, calc(4.6vw - 1.1px))`，算式與注意事項寫在 styles.scss 註解裡）。
  ⚠️ **改這句文案時每行要維持 20 字以內**，字數變多會靜默地又出現孤行，改完要在 360／375／390px 各看一次。
...
- **手機版商品陳列為雙欄緊湊版**（2026-09-18 主理人指定，規格表見 docs/COMPONENTS.md）：760px 以下所有商品區塊（選品陳列架、心頭好推薦牆、野餐季、秋日針織）一律「一排兩件」，
  以 390px（iPhone 12~16）為主力調校基準、向下相容 375／360px。**卡片維持無框**（奶油底直接放照片＋文字，勿改成白底方塊或加陰影）；
  商品名稱最多兩行、超出才收「…」（勿改回只留一行）；金額與幣別、分類標籤一律不斷行；手機版隱藏封面輪播圓點改顯示「第幾張／共幾張」；
  560px 以下全站 `.wrap` 側邊留白從 32px 漸進收窄到 20px（含導覽列、頁尾、各頁頁首）。
  改這些規格只改 styles.scss 的「手機版商品陳列：雙欄緊湊版」區塊；**情境短文 `.product-note` 的字級必須寫成 `.product-card .product-note`**，
  否則會被檔案最末的「全站內文字級規格化」蓋回 16px（選品陳列架的 `.shop-card .product-photo` 圓角同理，別在手機版另設）。
- 輪播兩種版型（2026-08-12 起**兩種都是整張滿版可點、不做實體按鈕、不疊光圈熱區**，跳轉目的地由 site.json 的 link 欄位決定，換網址只改 JSON）：**海報模式**（poster:true，圖片本身已含文案與按鈕設計；**2026-09-18 主理人指定：手機上整張完整露出、不再做 4:3 裁切**，整組都是海報時輪播會自動維持與桌機相同的 21:9，所以**新的海報圖請一律做成 21:9**，比例不同的圖在手機上仍會被裁；手機版 Banner 變矮後，左右箭頭會蓋住海報上的標題字，所以**手機版海報輪播不顯示箭頭、圓點改放在圖片下方**，切換靠點圓點或 5 秒自動輪播，桌機照舊）；**分割式版型**（非海報：左格紋色塊 theme butter/rose ＋右實拍，格紋色在 `--gingham-*`，手機上文下圖）。hotspot 熱區與 link_label 按鈕欄位已整組移除（勿重建，光圈會與圖上的按鈕對不準）；「遇見はむにぎり娃寶」Banner 已刪除（2026-08-12）；2026-08-28 起輪播為兩張海報：野餐季（連 `#picnic-plan`）＋針織開襟衫（連 `#knit-collection`），分割式版型暫無使用但勿刪程式。
...
styles.scss 的 breakpoint：**360 / 420 / 480 / 560 / 760（手機版商品雙欄的切換點）/ 820 / 900 / 1000px（導覽列收合為漢堡選單）**。
改版型後至少過一輪窄機（360 / 375 / 390px，手機版商品陳列的調校基準）、平板（760–820）、桌機（1440）。
Edge headless 用 `--screenshot` 單張模式最窄只到約 500px；要量 360~390px 請改用 CDP 的 `Emulation.setDeviceMetricsOverride`（做法見記憶檔）。
