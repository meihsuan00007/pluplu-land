...
| 規格清單 | 每個規格寫「供貨方式\|規格名稱」，多個規格用全形「；」分隔；售完的規格在名稱後加（售完）。例：`現貨\|櫻花粉；預購\|薄荷綠；現貨\|奶油白（售完）` | ✔ |
| 商品介紹 | 官網文案（禁用破折號） | ✔ |
| 小提醒 | 尺寸、材質注意事項，可留空 | |
...
```json
{
  ...
  "variants": [
    { "supply": "現貨", "name": "櫻花粉", "in_stock": true, "image": null },
    { "supply": "預購", "name": "薄荷綠", "in_stock": true, "image": null }
  ],
...
| 欄位 | 填什麼 |
|---|---|
| `id` | 商品編號，**兩位數字字串**（接續現有最大編號；目前到 47） |
...
| `price` / `original_price` | 純數字。沒特價時兩者填一樣 |
...
| `variants` | 規格陣列。`supply` 只能是 `"現貨"` 或 `"預購"`（出貨說明會自動對應）；`in_stock` 填 `false` 該規格會顯示售完不可選；`image` 有規格專屬圖就填路徑，沒有填 `null` |
