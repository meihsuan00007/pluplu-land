...
| `NEW_ARRIVAL_COUNT` / `coverPool()` / `pickCover()` / `coverKindOf()` | 首頁三大分類方塊的隨機封面（`core/covers.ts`）：新品上市＝目錄最新 5 件、織女手作＝tags 含織女手工系列、配件專區＝配件小物分類；**先平均抽商品、再抽該商品的主圖**（只認主圖，不從整本相簿抽），每次重新整理抽一張 | 首頁三大方塊的封面照片；換商品主圖＝換掉它在首頁可能出現的封面；後台 site.json 的圖片只在商品資料載入前顯示 |
...
| `pl-product-card` | `product-card.ts` | 商品卡片（首頁心頭好／選品陳列架共用；2026-08-25 起顯示價格；2026-08-31 起 `images` 多張時封面可輪播） | `data: ProductCardData`（`priceText` 有值才顯示價格，完售品項留空；`images` 給整本相簿＝封面可左右翻、圓點／計數、手機滑動、鍵盤左右鍵，點卡片開視窗停在同一張）、`showBuy`、`shopStyle`；`data.pid` 有值＝點卡片開詳情視窗；手機版（760px 以下）為雙欄緊湊版，規格見下方「手機版商品陳列規格」 |
...
| `covers.ts` | 首頁三大分類方塊的隨機封面取樣（先平均抽商品、再取該商品主圖；抽到的封面在同一次瀏覽中固定，重新整理才重抽） |
...
## 手機版商品陳列規格（2026-09-18 主理人指定）

**手機上所有商品區塊一律「一排兩件」**，規格集中在 `src/styles.scss` 的「手機版商品陳列：雙欄緊湊版」區塊，
`.product-grid`（選品陳列架、野餐季企劃、秋日針織企劃）與 `.product-grid--wall`（首頁大家的心頭好）共用同一套設定。

| 項目 | 規格 | 備註 |
| --- | --- | --- |
| 切換點 | 螢幕寬 760px 以下＝兩欄（原本 560px 以下會收成一欄，已取消） | 761px 以上（平板、桌機）維持原樣，不受影響 |
| 調校基準 | 390px（iPhone 12~16）為主，向下相容 375px（iPhone SE）與 360px（Android） | 實測 360／375／390／414 都不爆版、不出現左右捲動 |
| 卡片間距 | 左右 10px（≤560px 為 9px、≤360px 為 8px） | 上下留白較寬，維持品牌的呼吸感 |
| 卡片外觀 | **維持無框**：奶油底直接放照片＋文字，不加白底方塊、不加陰影 | 主理人指定，勿改成賣場平台那種白色方塊 |
| 商品主圖 | 1:1 正方形＋圓角 12px＋cover 置中裁切 | 與全站 `.product-photo` 同一套，手機版不另外改圓角 |
| 商品名稱 | 手機 14px（561~760px 的橫放手機／小平板放大為 15px），**最多兩行**，超出才在結尾收「…」 | 62 件商品中只有 7 件會被收尾；主理人指定不要改成「只留一行」 |
| 金額 | 幣別與數字永遠同一行（`white-space:nowrap`） | 「NT$ 390」絕不可拆行 |
| 分類標籤 | 單行膠囊，過長整條收「…」 | 不可斷成兩行 |
| 情境短文 | 12.5px（561~760px 為 13px），最多兩行 | 選擇器必須寫成 `.product-card .product-note`，否則會被檔案最末的「全站內文字級規格化」蓋回 16px |
| 購買按鈕 | 手機版改成整排置中（窄卡片放不下靠左的小膠囊） | 首頁野餐／針織／心頭好才有，選品陳列架本來就沒有 |
| 封面輪播 | 手機版隱藏圓點，改顯示右下角「第幾張／共幾張」 | 窄卡片上十個圓點會擠成一排；左右滑動與箭頭照常可用 |
| 點擊回饋 | 觸控裝置按下卡片會輕輕縮一下、放開回彈 | 桌機維持「滑過去照片緩緩放大」 |

側邊留白：560px 以下全站 `.wrap` 從 32px 漸進收窄到 20px（360px 時為 20px），兩欄卡片才有足夠寬度。
用漸進式而不是直接跳掉，是為了避免在某個寬度出現「畫面變寬、卡片反而變窄」的怪現象。
這是**全站性**的調整：手機上導覽列、頁尾、各頁頁首的左右留白也會跟著收窄約一個字寬。

`.product-name` 有 `min-height:3em`（＝預留兩行高度），所以同一排的兩張卡片，
就算一張名稱一行、一張兩行，價格與按鈕仍會對齊在同一水平線；名稱短的卡片下方會看到一小段留白，這是刻意保留的。
