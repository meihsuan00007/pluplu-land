...
@Component({
  ...
  imports: [NgTemplateOutlet, RouterLink, TitleBreakPipe],
  template: `
    <section class="carousel-shell" aria-label="主打活動輪播">
      <div class="wrap">
        <div class="carousel" (mouseenter)="stop()" (mouseleave)="start()">
              ...
              @if (s.poster) {
                @if (isExternal(s.link)) {
                  <a
                    ...
                    [attr.tabindex]="index() === $index ? null : -1"
                  >
                ...
                <!-- 分割式版型也是整張可點：不做實體按鈕，點任何位置都跳轉 link -->
                @if (isExternal(s.link)) {
                  <a
                    [href]="s.link"
                    target="_blank"
                    rel="noopener"
                    class="carousel-slide carousel-slide--split"
                    [class.is-active]="index() === $index"
                    [attr.tabindex]="index() === $index ? null : -1"
                  >
                    <ng-container *ngTemplateOutlet="splitInner; context: { s, i: $index }" />
                  </a>
                } @else {
                  <a
                    [routerLink]="posterRoute(s.link)"
          ...
          <ng-template #splitInner let-s="s" let-i="i">
            ...
            <div class="split-media">
              <!-- alt 留空：整張 Banner 是連結，標題已由 h2 朗讀，避免螢幕閱讀器唸兩次 -->
              <img [src]="img(s.image)" alt="" [attr.loading]="i === 0 ? null : 'lazy'" />
            </div>
          </ng-template>
